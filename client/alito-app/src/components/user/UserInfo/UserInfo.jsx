...
const UserInfo = () => {
    ...
    useEffect(()=>{
        ...
        fetch("http://localhost:8080/get-user-info-by-id", {
            ...
            .then(res => res.json())
            .then(res => {setUser(res); setFirstLetter(res.name.slice(0,1))})
    }, [])
    ...
    return (
        <div className={"user-info"}>
            ...
            <div className={"user-section"}>
                <Link to={"/profile/" + user?.id + "/ads"} style={pathname === "/profile/" + user?.id + "/ads" ? styleActiveLink : {}}>Объявления</Link>
                <Link to={"/profile/" + user?.id + "/reviews"} style={pathname === "/profile/" + user?.id + "/reviews" ? styleActiveLink : {}}>Отзывы</Link>
                {JSON.parse(localStorage.getItem('userInfo')).id === user?.id?
                    <>
                        <Link to={"/profile/" + user?.id + "/favorites"} style={pathname === "/profile/" + user?.id + "/favorites" ? styleActiveLink : {}}>Избранное</Link>
